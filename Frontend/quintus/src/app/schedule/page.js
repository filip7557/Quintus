    ...
    <>
      ...
      <main className={styles.container}>
        <div className={styles.card}>
          ...
          {pendingAppointments.length > 0 ? <div className={styles.pendingSection}><h2 className={styles.pendingTitle}>Termini na čekanju</h2><div className={styles.pendingList}>{pendingAppointments.map((appointment) => <button type="button" key={appointment.id} className={styles.pendingItem} onClick={() => openEdit(appointment)}><strong>{appointment.title}</strong>{appointment.notes ? <span className={styles.pendingNotes}>{appointment.notes}</span> : null}<small>Dodao/la: {appointment.createdByName}</small></button>)}</div></div> : null}
