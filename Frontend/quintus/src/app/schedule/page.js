...
function occursOnDay(appointment, dayKey) {
  ...
  if (end) return dayKey > startKey && dayKey <= dateKey(end);
...
export default function SchedulePage() {
  ...
  const load = async () => {
    ...
    if (rangeResponse?.status >= 200 && rangeResponse.status < 300) {
      setAppointments(
        (Array.isArray(rangeResponse.data) ? rangeResponse.data : []).map(
          normalize,
        ),
      );
    } else {
      setAppointments([]);
      setError(
        rangeResponse?.data?.message || "Greška pri dohvaćanju rasporeda.",
      );
    }
    if (pendingResponse?.status >= 200 && pendingResponse.status < 300) {
      setPendingAppointments(
        (Array.isArray(pendingResponse.data) ? pendingResponse.data : []).map(
          normalize,
        ),
      );
    } else {
  ...
  const openOwnerTransfer = async () => {
    ...
    setLoadingOwnerTransfer(true);
    const response = await getAppointmentOwners();
    if (response?.status >= 200 && response.status < 300) {
      ...
      setOwnerTransfer((transfer) =>
        transfer ? { ...transfer, candidates } : null,
      );
    } else {
      setOwnerTransfer(null);
      setError(response?.data?.message || "Greška pri dohvaćanju korisnika.");
    }
    setLoadingOwnerTransfer(false);
  };

  const confirmOwnerTransfer = async () => {
    if (!ownerTransfer?.ownerUserId) {
      setError("Odaberite novog vlasnika termina.");
      return;
    }
    setSavingOwnerTransfer(true);
    setError("");
    const response = await transferAppointmentOwner(
      ownerTransfer.appointmentId,
    ...
    return (
      <>
        ...
        <main className={styles.container}>
          <div className={styles.card}>Učitavanje...</div>
        </main>
      </>
    );
  }
      ...
      <>
        ...
        <main className={styles.container}>
          <div className={styles.card}>
            Nemate ovlasti za pristup rasporedu.
          </div>
        </main>
      </>
    );
  ...
  return (
    <>
      ...
      <main className={styles.container}>
        <div className={styles.card}>
            ...
            <div>
              ...
              <p className={styles.subtitle}>
                {viewMode === "month"
                  ...
                  : `Tjedan od ${displayDate(weekDays[0].date)} do ${displayDate(weekDays[6].date)}`}
              </p>
            </div>
          ...
          {pendingAppointments.length > 0 ? (
            <div className={styles.pendingSection}>
              <div
                ...
                onClick={() => {
                  if (!isMobile) setShowPending((value) => !value);
                }}
              >
                <h2 className={styles.pendingTitle}>
                  Termini na čekanju ({pendingAppointments.length})
                </h2>
                <button
                  type="button"
                ...
                <div className={styles.pendingList}>
                  {pendingAppointments.map((appointment) => (
                    <button
                      ...
                      style={{ "--pending-color": appointment.createdByColor }}
                      onClick={() => openEdit(appointment)}
                    >
                      <strong>{appointment.title}</strong>
                      {appointment.notes ? (
                        <span className={styles.pendingNotes}>
                          {appointment.notes}
                        </span>
                      ) : null}
                      <small>Dodao/la: {appointment.createdByName}</small>
                    </button>
                  ))}
                </div>
              ) : null}
          ...
          {loading ? (
            ...
            <div
              key={viewMode}
              className={`${styles.grid} ${styles.calendarView} ${viewMode === "month" ? styles.monthGrid : ""}`}
            >
              {calendarDays.map((day) => (
                ...
                >
                  <button
                    type="button"
                    className={`${styles.dayHeader} ${selectedDate === day.key ? styles.selected : ""}`}
                    onClick={() => openCreateForDay(day.key)}
                  >
                    <strong>
                      {viewMode === "month" ? day.date.getDate() : day.label}
                    </strong>
                    <span>
                      {viewMode === "month" ? day.label : displayDate(day.date)}
                    </span>
                  </button>
                  <div
                    className={styles.dayBody}
                    onClick={() => {
                      if (!isMobile) openCreateForDay(day.key);
                    }}
                  >
                    {appointments
                      .filter((appointment) =>
                        occursOnDay(appointment, day.key),
                      )
                      .map((appointment) => {
                        const editable =
                          String(appointment.createdByUserId).toLowerCase() ===
                        ...
                        return (
                          <button
                            ...
                            style={{
                              "--appointment-color": appointment.createdByColor,
                            }}
                            onClick={(event) => {
                              event.stopPropagation();
                            ...
                            title={
                              editable
                                ? "Uredi termin"
                                : "Termin drugog korisnika"
                            }
                          >
                            <span className={styles.time}>
                              {localTime(appointment.startAt)}
                              {appointment.endAt
                                ? `–${localTime(appointment.endAt)}`
                                : ""}
                            </span>
                            <strong>{appointment.title}</strong>
                            <small>{appointment.createdByName}</small>
                          </button>
                        );
                      })}
                  </div>
          ...
          <div className={styles.createActions}>
            <div className={styles.mobileCreateMenu} ref={createMenuRef}>
              <div
                className={`${styles.mobileCreateOptions} ${showCreateMenu ? styles.mobileCreateOptionsOpen : ""}`}
                role="menu"
                aria-hidden={!showCreateMenu}
              >
                <button
                  type="button"
                  className={`${styles.createButton} ${styles.mobileCreateOption}`}
                ...
                <button
                  type="button"
                  className={`${styles.secondaryCreateButton} ${styles.mobileCreateOption}`}
                  tabIndex={showCreateMenu ? 0 : -1}
                  onClick={() => {
                    setShowCreateMenu(false);
                    openCreatePending();
                ...
                >
                  Novi termin na čekanju
                </button>
              </div>
              <button
                type="button"
                className={`${styles.mobileCreateTrigger} ${showCreateMenu ? styles.mobileCreateTriggerOpen : ""}`}
                aria-label={
                  showCreateMenu
                    ? "Zatvori izbornik za dodavanje termina"
                    : "Dodaj termin"
                }
              ...
              >
                <span
                  className={styles.mobileCreateTriggerIcon}
                  aria-hidden="true"
                />
              </button>
            </div>
      ...
      {form ? (
        <div className={styles.overlay}>
          <form
            ...
            onSubmit={
              form.readOnly ? (event) => event.preventDefault() : submit
            }
          >
            <div className={styles.modalHeader}>
              <h2>
                {!form.id
                  ? "Novi termin"
                  : form.readOnly
                    ? `Termin – ${form.createdByName}`
                    : form.isPending
                      ? "Dovrši termin (na čekanju)"
                      : "Uredi termin"}
              </h2>
              <button
                type="button"
              ...
              <>
                ...
                <div className={styles.formRow}>
                  <label>
                    <span>
                      Datum završetka{" "}
                      <span className={styles.optional}>(opcionalno)</span>
                    </span>
                    <input
                      type="date"
                      lang="hr-HR"
                      ...
                      disabled={form.readOnly}
                      onChange={(event) =>
                        setForm({ ...form, repeatEndDate: event.target.value })
                      }
                    />
                  </label>
                  <label>
            ...
            <div className={styles.modalActions}>
              ...
              {form.id && isAdminOrOwner(currentUser) ? (
                <button
                  type="button"
                  className={styles.transferButton}
                  onClick={openOwnerTransfer}
                  disabled={saving}
                >
                  Promijeni vlasnika
                </button>
      ...
      {confirmingDelete ? (
        ...
        >
          <div
            ...
            role="alertdialog"
            aria-modal="true"
            aria-label="Potvrda brisanja termina"
            style={{ maxWidth: "420px" }}
          >
            <div className="modal-header">
              <h3>Obrisati termin?</h3>
              <button
                type="button"
      ...
      {ownerTransfer ? (
        <div className={styles.transferOverlay} role="presentation">
          <div
          ...
          >
            <div className={styles.transferHeader}>
              <h2 id="transfer-owner-title">Promijeni vlasnika termina</h2>
              <button
                type="button"
                className={styles.transferClose}
                onClick={() => setOwnerTransfer(null)}
                aria-label="Zatvori"
                disabled={savingOwnerTransfer}
              >
                ×
              </button>
            </div>
            <p className={styles.transferDescription}>
              Odaberite Ownera ili Workera koji preuzima termin:{" "}
              <strong>{ownerTransfer.title}</strong>
            </p>
            {loadingOwnerTransfer ? (
              <div className={styles.notice}>Učitavanje korisnika...</div>
            ) : (
              <div
                className={styles.ownerList}
                role="radiogroup"
                aria-label="Novi vlasnik termina"
              >
                {ownerTransfer.candidates.map((candidate) => (
                  <label key={candidate.id} className={styles.ownerOption}>
                    <input
                      type="radio"
                      name="appointment-owner"
                      value={candidate.id}
                      checked={
                        ownerTransfer.ownerUserId === String(candidate.id)
                      }
                      onChange={(event) =>
                        setOwnerTransfer({
                          ...ownerTransfer,
                ...
                {ownerTransfer.candidates.length === 0 ? (
                  <div className={styles.notice}>
                    Nema dostupnih Ownera ili Workera.
                  </div>
                ) : null}
              </div>
            )}
            <div className={styles.transferActions}>
              ...
              >
                {savingOwnerTransfer ? "Spremanje..." : "Potvrdi promjenu"}
              </button>
            </div>
          </div>
        </div>
